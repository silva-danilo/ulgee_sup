...
plot(z[,2], prob, type="n", ylim=c(0,0.7), xlab="gini", ylab="psewage adjusted", cex.axis=1.2, cex.lab=1.5)
polygon(c(z[,2], rev(z[,2])), c(prob.i, rev(prob.s)), col="red", border=NA)
...
prob.s <- exp(eta.s)/(1+exp(eta.s))
prob.i <- exp(eta.i)/(1+exp(eta.i))
plot(z[,4], prob, type="n", ylim=c(0,0.3), xlab="gini", ylab="psewage adjusted", cex.axis=1.2, cex.lab=1.5)
polygon(c(z[,4], rev(z[,4])), c(prob.i, rev(prob.s)), col="green", border=NA)
...
prob.s <- exp(eta.s)/(1+exp(eta.s))
prob.i <- exp(eta.i)/(1+exp(eta.i))
plot(z[,7], prob, type="n", ylim=c(0,0.5), xlab="gini", ylab="psewage adjusted", cex.axis=1.2, cex.lab=1.5)
polygon(c(z[,7], rev(z[,7])), c(prob.i, rev(prob.s)), col="blue", border=NA)
